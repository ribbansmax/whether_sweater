...
describe 'Forecast API' do
  it 'sends a forecast' do
    VCR.use_cassette('location_denver_both_calls') do
      ...
      expect(data[:attributes].keys).to eq(%i[current_weather daily_weather hourly_weather])
      expect(data[:attributes][:current_weather].keys).to eq(%i[datetime sunrise sunset temperature feels_like
      ...
      data[:attributes][:daily_weather].each do |day|
        expect(day.keys).to eq(%i[datetime sunrise sunset min_temp max_temp conditions icon])
      end
      expect(data[:attributes][:hourly_weather].length).to eq(8)
      data[:attributes][:hourly_weather].each do |hour|
        expect(hour.keys).to eq(%i[datetime temperature conditions icon])
      end
    end
  end
